...
export class ScheduleReviewComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    public editInfo(action) {
        try {
            console.log(action);
            this.router.navigate(['/workflow/schedule/csa/add-schedule/' + btoa(action)]);
        } catch (err) {
            this.global.addException('Schedule-review', 'editInfo()', err, { 'routeURL': '/workflow/schedule/csa/add-schedule/' + btoa(action) });
        }
    }
        ...
        if(!reqObj.scheduling_id){
          this.http.doPost('storeScheduling',reqObj,function(error: boolean, response: any){
            ...
            if(error){
              ...
              self.util.showDialog(DialogComponent, response.message, ['/workflow/schedule/csa/schedule-list/0']);
