...
export class AddTimelineScheduleComponent implements OnInit {
    ...
    ngOnInit() {
        console.log('AddTimelineScheduleComponent');
            ...
            this.http.doGet('assets', function(error: boolean, response: any){
                ...
                if( error ){
                    ...
                    for(let i=0; i<response.data.length; i++){
                        ...
                        if(response.data[i].data.asset_id == self.resourceId){
                            console.log("after if ====",response.data[i].data.asset_id + ' === ' + self.resourceId);
                            self.getSelectedAsset(response.data[i].data,0);
                            self.personDetails = null;
                        }
                    }
            ...
            this.http.doGet('getEmployee', function(error: boolean, response: any){
                ...
                if( error ){
                    ...
                    for(let i=0; i<response.data.length; i++){
                        ...
                        //console.log("before if ====",response.data[i].employee_info.id + ' === ' + self.resourceId);
                        if(response.data[i].employee_info.id == self.resourceId){
                            //console.log("after if ====",response.data[i].employee_info.id + ' === ' + self.resourceId);
                            self.getSelectedPerson(response.data[i].employee_info);
                        }
                    }
    ...
    addScheduleForAssetForm(option, formVal:any = {}){
       console.log('formVal ==> ',formVal);
        this.scheduleForAssetForm = this.fb.group({
            ...
            asset_name: new FormControl(option == '0' ? '' : this.assetDetails ? this.assetDetails.short_tag : null,[]),
    ...
    next(action): void {
        ...
        if(this.scheduleForAssetForm.valid && !this.timeNowError){
            ...
            console.log("reviewObj-1",reviewObj);
            if(this.scheduleType == 'recurring'){
            ...
            this.http.doPost('check-work-orders', reqObj, function (error: boolean, response: any) {
                ...
                if (error) {
                    ...
                    if (response.data.work_orders.length == 0 && ((self.scheduleFor == 'Asset' && response.data.persons.length == 0) || (self.scheduleFor == 'person' && response.data.assets.length == 0))) {
                        ...
                        let resourceName = self.scheduleFor == 'Asset' ? (self.assetDetails ? self.assetDetails.full_name : '') : self.personDetails.staff_name;
                        ...
                        self.util.showDialog(ScheduleDialogComponent, resourceName + " is assigned to the following " + msgStr + ":", ['/workflow/schedule/csa/timeline-schedule-review/' + btoa(action)], "Scheduling Confirmation", "CONFIRMATION_WITH_WARNING", { 'type': self.scheduleFor, 'details': response.data });
                    }
                }
            });
        } catch (err) {
