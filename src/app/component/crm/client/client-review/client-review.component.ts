...
//import { SupplierDialog } from '../supplier-dialog.component';
declare var $ :any;
...
export class ClientReviewComponent implements OnInit {
    ...
    addCleint(){
        ...
        delete reqObject.ref_type_name;
        for(let i=0; i<reqObject.contacts.length; i++){
            reqObject.contacts[i].phone_no = self.util.unMaskPhoneNumber(reqObject.contacts[i].phone_no);
            for( let j=0;j<reqObject.contacts[i].address.length; j++){
                reqObject.contacts[i].address_index_id[j] = reqObject.contacts[i].address[j]-1;
            }
            reqObject.contacts[i].address = JSON.stringify(reqObject.contacts[i].selAdd);
            delete reqObject.contacts[i].selAdd;
        }
        for(let i=0; i<reqObject.address.length; i++){
            reqObject.address[i].address_type = JSON.stringify(reqObject.address[i].selVal.selectedAddressType);
            delete reqObject.address[i].selVal;
        }
        console.log("reqObject= ",reqObject);
        console.log(JSON.stringify(reqObject));
        self.util.addSpinner('addCleint', "Submit");
        this.http.doPost('clients',reqObject,function(error: boolean, response: any){
        self.util.removeSpinner('addCleint', "Submit");
            if( error ){
                ...
                console.log(response.message);
                //self.dialog.open(SupplierDialog, { data: { 'action': 'createSuccess', 'successMsg': response.message } });       
                self.router.url.split('/')[2]=='csa-onboarding' ? self.router.navigate(['/csa-onboarding/guide']) : self.router.navigate(['/crm/csa/client-list/0']);
            }        
        });
    }
